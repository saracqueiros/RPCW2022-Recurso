    ...
    if (req.query.groupBy != null) {
        ...
        Movimento.listar()
            .then(dados => {
                ...
                res.render('movimentos', { dados: dados })
            })
            .catch(e => res.status(500).jsonp({ error: e }))
    }
...
router.get('/api/pagamentos/:id', function(req, res, next) {
    ...
    Pagamento.listarPagamentos(req.params.id)
        .then(dados => {
            ...
            meses = [dados.Jan, dados.Fev, dados.Mar, dados.Abr, dados.Mai, dados.Jun, dados.Jul, dados.Ago, dados.Set, dados.Out, dados.Nov, dados.Dez]
...
router.get('/api/pagamentos', function(req, res, next) {
    Pagamento.listar()
        .then(dados => res.render('pagamentos', { dados: dados }))
        .catch(e => res.status(500).jsonp({ error: e }))
});
